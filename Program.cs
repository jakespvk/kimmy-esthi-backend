...
admin.MapPost("/", async ([FromBody] LoginRequest loginRequest, AppointmentDb db) =>
{
    ...
    {
        var user = await db.AdminUsers.Where(x => x.Username == loginRequest.Username).FirstAsync();
        // user.Token = "super-secret-and-special-token-anya-forger-is-the-voice-im-hearing-in-my-head-when-i-type-that";
        user.Token = new Random(1000000000).GetHashCode().ToString() + new Random(1000000000).GetHashCode().ToString();
        await db.SaveChangesAsync();
...
admin.MapGet("/{token}/appointments", async ([FromQuery] bool booked, [FromQuery] DateTime? date, string token, AppointmentDb db) =>
{
    if (!await db.AdminUsers.AnyAsync(x => x.Token == token))
    {
        Results.Challenge();
    ...
    {
        return await db.Appointments.Where(x => x.Status == AppointmentStatus.Booked && x.DateTime.Date == ((DateTime)date).Date).Include(x => x.ScheduledAppointment).OrderBy(x => x.DateTime).ToListAsync();
    }
    else if (booked)
    {
        return await db.Appointments.Where(x => x.Status == AppointmentStatus.Booked).Include(x => x.ScheduledAppointment).OrderBy(x => x.DateTime).ToListAsync();
    }
    else if (date is not null)
    {
        return await db.Appointments.Where(x => x.DateTime.Date == ((DateTime)date).Date).Include(x => x.ScheduledAppointment).OrderBy(x => x.DateTime).ToListAsync();
    }
